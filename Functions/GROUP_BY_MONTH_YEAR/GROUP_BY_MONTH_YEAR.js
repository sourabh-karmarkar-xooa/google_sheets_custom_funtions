 ...
 * If the result needs to be filtered further specify the collumn number in the filter_col_number parameter along with the value that needs to be matched in the filter_text parameter.
 *
 ...
 * @param {String} filter_text The string value which will be used to include only the results that match the value.
  ...
  try {
    ...
    for (let i = 0; i < range.length; i++) {
      ...
      if (filter_text && filter_col_number) {
        ...
        if (record[filter_col_number - 1] !== filter_text) continue;
